# 🎯 DSA Skills & Concepts Tracker

> **Last Updated:** Tuesday, August 18, 2026

---

## 📊 Quick Stats

- **Total Concepts Mastered:** 15
- **Most Used Concept:** HashMap (5 times)
- **Recently Learned:** Euclidean Algorithm (GCD) (2026-08-18)
- **Average Reuse Rate:** 1.9 times per concept

---

## 🛠️ Skills Tracking Table

| Concept/Pattern | Category | First Used | First Problem | Times Used | Usage History |
|-----------------|----------|------------|---------------|------------|---------------|
| HashMap | Data Structure | 2026-02-13 | Two Sum (#1) | 5 | Two Sum (#1) (2026-02-13), Group Anagrams (#49) (2026-02-13), Top K Frequent Elements (#347) (2026-02-15), Valid Sudoku (#36) (2026-02-27), Encode and Decode Strings (#271) (2026-03-02) |
| HashSet | Data Structure | 2026-02-13 | Contains Duplicate (#217) | 2 | Contains Duplicate (#217) (2026-02-13), Valid Sudoku (#36) (2026-02-27) |
| Frequency Array | Pattern | 2026-02-13 | Valid Anagram (#242) | 2 | Valid Anagram (#242) (2026-02-13), Valid Sudoku (#36) (2026-02-27) |
| Sorting (Arrays.sort) | Algorithm | 2026-02-13 | Group Anagrams (#49) | 1 | Group Anagrams (#49) (2026-02-13) |
| Kadane's Algorithm | Pattern | 2026-02-15 | Maximum Subarray (#53) | 2 | Maximum Subarray (#53) (2026-02-15), Maximum Product Subarray (#152) (2026-02-18) |
| Prefix Product | Pattern | 2026-02-18 | Product of Array Except Self (#238) | 1 | Product of Array Except Self (#238) (2026-02-18) |
| In-place Operation | Optimization | 2026-02-18 | Product of Array Except Self (#238) | 4 | Product of Array Except Self (#238) (2026-02-18), Rotate Array (#189) (2026-02-26), Kth Largest Element in an Array (#215) (2026-07-06), String Compression (#443) (2026-08-18) |
| Constant Space | Optimization | 2026-02-18 | Product of Array Except Self (#238) | 4 | Product of Array Except Self (#238) (2026-02-18), Rotate Array (#189) (2026-02-26), Kth Largest Element in an Array (#215) (2026-07-06), String Compression (#443) (2026-08-18) |
| Array Reversal | Pattern | 2026-02-26 | Rotate Array (#189) | 1 | Rotate Array (#189) (2026-02-26) |
| Priority Queue (Heap) | Data Structure | 2026-02-15 | Top K Frequent Elements (#347) | 1 | Top K Frequent Elements (#347) (2026-02-15) |
| Quickselect | Algorithm | 2026-07-06 | Kth Largest Element in an Array (#215) | 1 | Kth Largest Element in an Array (#215) (2026-07-06) |
| Two Pointers | Pattern | 2026-08-18 | Merge Strings Alternately (#1768) | 2 | Merge Strings Alternately (#1768) (2026-08-18), String Compression (#443) (2026-08-18) |
| Euclidean Algorithm (GCD) | Algorithm | 2026-08-18 | Greatest Common Divisor of Strings (#1071) | 1 | Greatest Common Divisor of Strings (#1071) (2026-08-18) |
| Recursion | Algorithm | 2026-08-18 | Greatest Common Divisor of Strings (#1071) | 1 | Greatest Common Divisor of Strings (#1071) (2026-08-18) |
| Concatenation Commutativity Check | Pattern | 2026-08-18 | Greatest Common Divisor of Strings (#1071) | 1 | Greatest Common Divisor of Strings (#1071) (2026-08-18) |

---

## 📂 Category Breakdown

### Data Structures
*Track: Arrays, HashMaps, Sets, Stacks, Queues, Heaps, Trees, Graphs, etc.*

| Concept | First Used | Times Used | Latest Use |
|---------|------------|------------|------------|
| HashMap | 2026-02-13 | 5 | Encode and Decode Strings (#271) (2026-03-02) |
| HashSet | 2026-02-13 | 2 | Valid Sudoku (#36) (2026-02-27) |
| Priority Queue (Heap) | 2026-02-15 | 1 | Top K Frequent Elements (#347) (2026-02-15) |

### Algorithms
*Track: Sorting, Searching, Recursion, Backtracking, DFS, BFS, etc.*

| Concept | First Used | Times Used | Latest Use |
|---------|------------|------------|------------|
| Sorting (Arrays.sort) | 2026-02-13 | 1 | Group Anagrams (#49) (2026-02-13) |
| Quickselect | 2026-07-06 | 1 | Kth Largest Element in an Array (#215) (2026-07-06) |
| Euclidean Algorithm (GCD) | 2026-08-18 | 1 | Greatest Common Divisor of Strings (#1071) (2026-08-18) |
| Recursion | 2026-08-18 | 1 | Greatest Common Divisor of Strings (#1071) (2026-08-18) |

### Patterns & Techniques
*Track: Two Pointers, Sliding Window, Prefix Sum, Kadane's, Fast-Slow Pointers, etc.*

| Concept | First Used | Times Used | Latest Use |
|---------|------------|------------|------------|
| Frequency Array | 2026-02-13 | 2 | Valid Sudoku (#36) (2026-02-27) |
| Kadane's Algorithm | 2026-02-15 | 2 | Maximum Product Subarray (#152) (2026-02-18) |
| Prefix Product | 2026-02-18 | 1 | Product of Array Except Self (#238) (2026-02-18) |
| Array Reversal | 2026-02-26 | 1 | Rotate Array (#189) (2026-02-26) |
| Two Pointers | 2026-08-18 | 2 | String Compression (#443) (2026-08-18) |
| Concatenation Commutativity Check | 2026-08-18 | 1 | Greatest Common Divisor of Strings (#1071) (2026-08-18) |

### Time/Space Optimization
*Track: In-place operations, Constant space, Memoization, etc.*

| Concept | First Used | Times Used | Latest Use |
|---------|------------|------------|------------|
| In-place Operation | 2026-02-18 | 4 | String Compression (#443) (2026-08-18) |
| Constant Space | 2026-02-18 | 4 | String Compression (#443) (2026-08-18) |

---

## 🔥 Mastery Levels

**Legend:**
- 🌱 **Beginner** (1-2 uses): Just learned, needs more practice
- 🌿 **Intermediate** (3-5 uses): Getting comfortable, building confidence
- 🌳 **Proficient** (6-10 uses): Solid understanding, can apply reliably
- 🏆 **Master** (11+ uses): Expert level, second nature

| Mastery Level | Concepts | Count |
|---------------|----------|-------|
| 🏆 Master (11+) | *None yet* | 0 |
| 🌳 Proficient (6-10) | *None yet* | 0 |
| 🌿 Intermediate (3-5) | HashMap, In-place Operation, Constant Space | 3 |
| 🌱 Beginner (1-2) | HashSet, Frequency Array, Sorting, Kadane's Algorithm, Prefix Product, Array Reversal, Priority Queue (Heap), Quickselect, Two Pointers, Euclidean Algorithm (GCD), Recursion, Concatenation Commutativity Check | 12 |

---

## 📈 Concept Usage Trends

### Top 5 Most Used Concepts
1. **HashMap** - 5 times (🌿 Intermediate)
2. **In-place Operation** - 4 times (🌿 Intermediate)
3. **Constant Space** - 4 times (🌿 Intermediate)
4. **Kadane's Algorithm** - 2 times (🌱 Beginner)
5. **HashSet** - 2 times (🌱 Beginner)

### Recently Learned (Last 5)
1. **Euclidean Algorithm (GCD)** (2026-08-18) - Greatest Common Divisor of Strings
2. **Recursion** (2026-08-18) - Greatest Common Divisor of Strings
3. **Concatenation Commutativity Check** (2026-08-18) - Greatest Common Divisor of Strings
4. **Two Pointers** (2026-08-18) - Merge Strings Alternately
5. **Quickselect** (2026-07-06) - Kth Largest Element in an Array

---

## 💡 Usage Instructions

### Automatic Tracking
The Skills Tracker is **automatically updated** when you use these commands:
- `/analyse` - Detects concepts used in your solution
- `/update` - Confirms and records concepts in tracker
- `/summarise` - Reviews concepts learned in current problem

### Manual Tracking (if needed)
If you want to manually add a concept:
```
Concept: [Name]
Category: [Data Structure/Algorithm/Pattern/Optimization]
First Used: [Date] - [Problem Name (#Number)]
Usage History: [Problem1 (Date)], [Problem2 (Date)], ...
```

### View Skills Status
Use `/current` command to see:
- Total concepts mastered
- Most frequently used concepts
- Recent skill additions
- Mastery distribution

---

## 🎯 Skill Development Goals

### Concepts to Master Next
*Based on common patterns in top interview problems:*

1. **Heap/Priority Queue** - For top-K problems, scheduling, merge operations
2. **Two Pointers** - For array manipulation, string problems
3. **Sliding Window** - For substring/subarray problems
4. **Binary Search** - For search problems, optimization
5. **Dynamic Programming** - For optimization problems
6. **DFS/BFS** - For tree/graph traversal
7. **Backtracking** - For combinatorial problems
8. **Trie** - For prefix/dictionary problems

---

## 📝 Notes

- **First Use** = The problem where you first successfully implemented the concept
- **Times Used** = Total number of problems where you applied this concept (including first use)
- **Usage History** = Chronological list: "Problem Name (YYYY-MM-DD), ..."
- Concepts are automatically detected from your solution code
- Only concepts YOU implemented count (not AI-generated test code)

---

*Last tracking update: Tuesday, August 18, 2026 - String Compression (#443) reused Two Pointers (now 2 uses), In-place Operation and Constant Space (now 4 uses each) — no new concepts introduced*
